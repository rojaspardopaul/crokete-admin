...
const useProductFilter = (data) => {
  ...
  const handleUploadProducts = () => {
    ...
    if (newProducts.length < 1) {
      notifyError("Por favor, suba o seleccione un archivo CSV primero!");
    } else {
      if (handleDisableForDemo()) {
        return; // Exit the function if the feature is disabled
      }
  ...
  const handleUploadMultiple = (e) => {
    ...
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        ...
        setLoading(false);
        notifyError("Por favor, ingresar datos válidos!");
      }
    } else {
      setLoading(false);
      notifyError("Por favor, suba o seleccione un archivo CSV/JSON valido!");
    }
  };
