...
const useFilter = (data) => {
    ...
    if (language) {
      services = services.filter(
        (lan) =>
          ...
          lan.code?.toLowerCase().includes(language.toLowerCase()) ||
  ...
  const handleUploadProducts = () => {
    if (newProducts.length < 1) {
      notifyError("Por favor, cargue o seleccione un archivo CSV primero!");
    } else {
  ...
  const handleSelectFile = (e) => {
    ...
    if (file && file.type === "application/json") {
      ...
      notifyError("Tipo de archivo no soportado!");
    }
  };
  ...
  const handleUploadMultiple = (e) => {
    ...
    if (selectedFile.length > 1) {
      ...
      notifyError("Por favor, cargue o seleccione un archivo JSON/CSV/XLS primero!");
    }
  };
